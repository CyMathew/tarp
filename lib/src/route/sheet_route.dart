...
class SheetRoute<T> extends PageRoute<T> with DelegatedTransitionsRoute<T> {
  ...
  /// Drag threshold to block any interaction if [Route.popDisposition] returns [RoutePopDisposition.doNotPop]
  /// See also:
  ...
  Widget buildSheet(BuildContext context, Widget child) {
    SheetPhysics? effectivePhysics = SnapSheetPhysics(
      detents: detents ?? <double>[Detents.none, Detents.large],
      parent: physics,
    );
    if (!draggable) {
...
/// Predefined stops for modals that allow modals to snap to certain heights
class Detents {
  /// Modal detent to near-top of screen
  static const double large = 1.0;

  /// Modal detent for middle of the screen
  static const double medium = 0.5;

  /// Modal detent for no visible modal (closed modal)
