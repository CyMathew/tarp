...
/// of closest value inside the [detents] list
///
...
class SnapSheetPhysics extends ScrollPhysics with SheetPhysics {
  ...
  bool shouldReload(covariant ScrollPhysics old) {
    return old is SnapSheetPhysics &&
        (old.relative != relative || !listEquals(old.detents, detents));
  }
  ...
  double _getTargetPixels(
    ...
    final double targetPixels =
        getPixelsFromPage(position, page.clamp(0, detents.length - 1));
    ...
    return getPixelsFromPage(position, page.clamp(0, detents.length - 1));
  }
  ...
  int _nearestStopForExtent(double extent) {
    ...
    final int stop = detents.asMap().entries.reduce(
  ...
  double getPixelsFromPage(ScrollMetrics position, int page) {
    if (detents[page].isInfinite) {
      return position.maxScrollExtent;
    }
    return detents[page] * extentFor(position);
  }
